...
$appProject = Get-Content -Raw -LiteralPath (Join-Path $projectRoot 'src\HuahaiClipboard.App\HuahaiClipboard.App.csproj')
...
if ($appProject -notmatch '<WindowsAppSDKSelfContained>true</WindowsAppSDKSelfContained>') { throw 'The app must carry its Windows App SDK runtime files.' }
if ($buildScript -match 'WindowsAppRuntimeInstall') { throw 'A self-contained Windows App SDK build must not package the external Windows App Runtime installer.' }
...
if ($bootstrapper -match 'Microsoft\.WindowsAppRuntime\.1\.7|WindowsAppRuntimeInstall-x64') { throw 'Setup must not depend on a machine-installed Windows App Runtime.' }
