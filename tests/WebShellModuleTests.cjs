...
test('interaction adapter exposes every WebView-visible control exactly once', () => {
  ...
  assert.equal(staticIds.length, 44);
  ...
  assert.equal(staticIds.length + Object.keys(interaction.filterControls).length + Object.keys(interaction.recordControls).length + Object.keys(interaction.exclusionControls).length, 55);
});
