...
if (([regex]::Matches($shell, '1\.1\.8')).Count -lt 2 -or $shell -match '1\.1\.7') { throw 'About page version is not 1.1.8.' }
if ($installer -notmatch 'AssemblyVersion\("1\.1\.8\.0"\)' -or
    $installer -notmatch 'AssemblyFileVersion\("1\.1\.8\.0"\)' -or
    $installer -notmatch 'SetValue\("DisplayVersion", "1\.1\.8"') {
    throw 'Installer metadata version is not 1.1.8.'
}
if ($readme -notmatch 'Version=1\.1\.8' -or $readme -notmatch 'webview-build-1\.1\.8') {
    throw 'README release commands are not pinned to 1.1.8.'
}
