...
internal static class InstallTargetPolicy
{
    ...
    internal static void Validate(string installRoot, string registeredInstallRoot)
    {
        ...
        if (entries.Length == 1 && Directory.Exists(entries[0]) &&
            String.Equals(Path.GetFileName(entries[0]), "Data", StringComparison.OrdinalIgnoreCase))
            return;
        ...
        if (File.Exists(Path.Combine(target, OwnerMarkerName))) return;
