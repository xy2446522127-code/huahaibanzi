(function (global, factory) {
  ...
  const staticControls = Object.freeze({
    'panel.search': '#searchInput', 'panel.minimize': '#minimizeButton', 'panel.settings': '#settingsButton', 'records.scroll': '#recordList', 'panel.autohide': '#autoHide', 'panel.drag': '.panel-header',
    ...
    'appearance.opacity': '#opacityRange', 'appearance.scale': '#scaleRange', 'appearance.reset-scale': '#resetScale',
  ...
  function markRecord(root = document) {
    const row = root.querySelector('.record');
    if (!row) return;
  ...
  return Object.freeze({ staticControls, filterControls, recordControls, markStatic, markFilters, markRecord });
});
